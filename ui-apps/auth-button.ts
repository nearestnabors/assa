...
// Step 2: User confirms they authorized - check status
  ...
  try {
    ...
    if (textContent && 'text' in textContent) {
      ...
      if (text.includes('connected')) {
        ...
        statusText.textContent = 'Not detected. Try again.';
        (checkBtn as HTMLButtonElement).disabled = false;
      }
    }
  } catch (error) {
    statusText.textContent = `Error: ${error instanceof Error ? error.message : 'Unknown'}`;
    (checkBtn as HTMLButtonElement).disabled = false;
  }
});
