...
function attachEventListeners(): void {
  ...
  for (const btn of document.querySelectorAll('[data-action="reply"]')) {
    // biome-ignore lint/complexity/noExcessiveCognitiveComplexity: UI flow with auth handling
    btn.addEventListener("click", async (e) => {
        ...
        if (successData.success) {
          ...
          setTimeout(async () => {
            try {
              ...
              // Update button state based on remaining items
              if (currentData) {
                currentData.totalCount = Math.max(
                  0,
...
app.ontoolresult = async () => {
  ...
  try {
    ...
    if (textContent && "text" in textContent) {
      ...
      ) as ConversationsData & { error?: boolean; message?: string };
...
loadMoreBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    loadMoreBtn.textContent =
      currentOffset > 0 ? "Error - try again" : "Check for new mentions";
  } finally {
